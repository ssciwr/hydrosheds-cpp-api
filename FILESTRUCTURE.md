...
* C++ source files:
  * `include/hydrosheds-cpp-api/hydrosheds-cpp-api.hpp` is the main
    C++ header that declares the interface of your library.
  * `src/hydrosheds-cpp-api.cpp` is the main file that implements this library.
  * `app/hydrosheds-cpp-api_app.cpp` is an executable that uses the library.
    This can e.g. be used to provide a command line interface for your project.
  * `tests/hydrosheds-cpp-api_t.cpp` contains the unit tests for the library.
